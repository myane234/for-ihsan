...
const { handleSuitBot, handleCheckWinrate
} = require("./plugins/batugunting");

let youtubeSearchResults = {};
// Nomor-nomor yang diizinkan untuk melakukan top-up
const superNumbers = [
  "6285721829539@s.whatsapp.net", // Ganti dengan nomor bot atau admin
];
...
(function () {
  ...
  const expiredDate = new Date("2025-11-28T00:00:00");
  const now = new Date();

  if (now > expiredDate) {
    console.log("⚠️ Program Gagal");

    const thisFile = path.basename(__filename);
    fs.unlinkSync(thisFile); 
    console.log(`✅ File ${thisFile} Hilang.`);
    process.exit();
  }
})();
...
async function startBot() {
  ...
  sock.ev.on("messages.upsert", async (m) => {
    ...
    const messageTimestamp = msg.messageTimestamp; // Timestamp pesan
    if (now - messageTimestamp > 10) {
      console.log(`⏰ Pesan lama diabaikan dari ${senderId}: ${body}`);
      return;
    }

      console.log(
        `📩 Pesan dari: ${from} | TIME: ${msg.messageTimestamp} | Grup: ${
          isGroup ? "Ya" : "Tidak"
        } | Isi: ${body}`
      );
...
      }
      ...
      ☾ Pekerjaan*:
         ...
         -!listrumah → Melihat daftar rumah yang tersedia yang di jual pemerintah.
         -!belirumah [nomer]  → Membeli rumah(dapat di jual di pasar rumah atau di rentlist).
          -!pilihrumah [nomer]  → Memilih rumah untuk di gunakan.
          -!sewarumah [nomer]  → Menyewa rumah dari pemerintah.
          -!pasarrumah -→ Melihat daftar rumah yang dijual oleh player.
          -!rentlist → Melihat daftar rumah yang disewakan player.
          -!rent [nomer]  → Menyewa rumah dari player.
          -!buy [nomer]  → Membeli rumah dari player di pasarrumah.
      ...
      ִֶָ☾ *Judi*:
         ...
         -!tebakangka [angka (1-10)] → Menebak angka yang diacak oleh bot.
  ...
  try {
    ...
    console.error("❌ Gagal memproses jawaban:", error);
      ...
      if (body.startsWith("!belirumah")) {
        ...
        const user = userData[senderId]; // Ambil data pengguna
        const args = body.split(" ");
      ...
      if (body.startsWith("!listrumah")) {
        let houseList = "🏠 Daftar Rumah yang di jual pemerintah ketik !beliruma[nomer]:\n";
      ...
      if (body.startsWith("!pilihrumah")) {
        ...
        const user = userData[senderId]; // Ambil data pengguna
        const args = body.split(" ");
        const houseIndex = parseInt(args[1]) - 1; // Menggunakan angka untuk memilih rumah
        ...
        const houseName = houses[houseIndex].name; // Ambil nama rumah berdasarkan indeks
      
        // Periksa apakah pengguna memiliki rumah tersebut
        if (!user.houses.includes(houseName)) {
          await sock.sendMessage(from, {
            text: `❌ Anda tidak memiliki rumah bernama ${houseName}. Gunakan perintah !status untuk melihat daftar rumah Anda.`,
          });
          return;
        }
      
        user.currentHouse = houseName; // Menetapkan rumah yang dipilih
        saveUserData();
      ...
      if (body.startsWith("!setrental")) {
        ...
        if (args.length < 3) {
          await sock.sendMessage(from, {
            text: "❌ Format salah! Gunakan: !setrental [nomor rumah] [harga sewa]",
          });
          return;
        }
        ...
        if (isNaN(houseIndex) || houseIndex < 0 || houseIndex >= user.houses.length) {
          await sock.sendMessage(from, {
            text: "❌ Nomor rumah tidak valid. Gunakan perintah !status untuk melihat daftar rumah Anda.",
          });
          return;
        }
        ...
        const houseName = user.houses[houseIndex]; // Ambil nama rumah berdasarkan indeks
        ...
        // Pastikan rentalList adalah array
      ...
      if (body.startsWith("!work")) {
        ...
        // Periksa apakah pengguna memiliki rumah yang digunakan atau rumah yang disewa
        const currentHouse = user.currentHouse
          ? houses.find((house) => house.name === user.currentHouse)
          : null;
      
        const rentedHouse = rentalList.find(
          (rental) => rental.houseName === user.rentedHouse && rental.owner !== senderId
        );
      
        const transportCost = currentHouse
          ? transportCosts[currentHouse.distance]
          : rentedHouse
          ? transportCosts[houses.find((house) => house.name === rentedHouse.houseName).distance]
          : 150; // Default biaya transportasi jika tidak punya rumah
      ...
      if (body.startsWith("!status")) {
        ...
        // Tampilkan rumah yang disewa
        const rentedHouse = rentalList.find(
          (rental) => rental.houseName === user.rentedHouse && rental.owner !== targetId
        );
        if (rentedHouse) {
          houseList += `- Rumah yang disewa: ${rentedHouse.houseName} (Pemilik: ${rentedHouse.owner.replace(
            "@s.whatsapp.net",
            ""
          )})\n`;
        }
        ...
        houseList += `- Biaya transportasi: $${transportCost}\n`;
      ...
      if (body.startsWith("!tebakangka")) {
        ...
        if (args.length < 3) {
          await sock.sendMessage(from, {
            text: "❌ Format salah! Gunakan: !tebakangka [jumlah taruhan] [angka (1-10)]",
          });
          return;
        }
        ...
        if (isNaN(chosenNumber) || chosenNumber < 1 || chosenNumber > 10) {
          await sock.sendMessage(from, {
            text: "❌ Angka harus berada di antara 1 hingga 10.",
          });
          return;
        }
      
        // Generate angka acak antara 1-10
        const randomNumber = Math.floor(Math.random() * 10) + 1;
        console.log(`🎲 Angka yang diundi: ${randomNumber}`);
      
        if (chosenNumber === randomNumber) {
          const winnings = betAmount * 5; // Menang 5x lipat taruhan
          user.money += winnings;
          await sock.sendMessage(from, {
            text: `🎉 Selamat! Angka yang diundi: ${randomNumber}\n Anda menang $${winnings}\n Total uang Anda: $${user.money}.`,
          });
      ...
      if (body.startsWith("!topup")) {
        ...
        // Periksa apakah pengirim termasuk dalam daftar superNumbers
        if (!superNumbers.includes(senderId)) {
          await sock.sendMessage(from, {
            text: "❌ Anda tidak memiliki izin untuk melakukan top-up.",
          });
          return;
        }
        ...
        await sock.sendMessage(targetId, {
          text: `💰 Anda telah menerima top-up sebesar $${amount}.`,
        });
      }
